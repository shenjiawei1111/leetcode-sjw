...
- **学习内容**：回溯算法
- **完成题目**：
  1. 电话号码的组合：使用回溯的思想，通过递归的方式遍历所有可能的字母组合。首先定义一个映射表存储每个数字对应的字母，然后从第一个数字开始，递归地尝试所有可能的字母，直到处理完所有数字，将得到的组合加入结果列表。
  2. 组合总和：同样使用回溯的思想，对于给定的候选数组和目标值，通过递归的方式尝试所有可能的组合。在递归过程中，维护当前组合和当前组合的和，若当前和等于目标值，则将该组合加入结果列表；若当前和大于目标值，则停止递归；否则继续尝试添加候选数。
  3. 括号生成：使用回溯的思想，递归生成所有有效的括号组合。在递归过程中，维护当前的括号字符串、左括号和右括号的数量。若左括号数量小于总括号对数，则可以添加左括号；若右括号数量小于左括号数量，则可以添加右括号。当左右括号数量都达到总括号对数时，将当前字符串加入结果列表。

### 10月15日
- **学习内容**：回溯算法
- **完成题目**：
  1. 单词搜索：使用深度优先搜索（DFS）和回溯的方法。在二维网格中遍历每个单元格作为起点，当匹配到第一个字符后，向上下左右四个方向递归搜索下一个字符，在搜索过程中标记已访问的单元格，回溯时撤销标记，若能匹配完所有字符则找到单词。
  2. 分割回文串：使用回溯的方法。从字符串的起始位置开始，枚举所有可能的分割点，判断分割出的子串是否为回文串，若是则将其加入路径，继续递归处理剩余子串，回溯时撤销选择，最终收集所有符合条件的分割方案。

### 10月16日
- **学习内容**：动态规划应用
- **完成题目**：
  1. 最长回文子串：使用动态规划和寻找中心的方法来解决。动态规划方法通过构建二维数组 `dp`，`dp[i][j]` 表示字符串从 `i` 到 `j` 的子串是否为回文串，根据状态转移方程更新 `dp` 数组以找到最长回文子串。寻找中心的方法是遍历每个字符，以该字符或相邻两个相同字符为中心向两边扩展，记录最长回文子串。
  2. 最长公共子序列：使用动态规划的方法。构建二维数组 `dp`，`dp[i][j]` 表示字符串 `A` 的前 `i` 个字符和字符串 `B` 的前 `j` 个字符的最长公共子序列长度。根据字符是否相等来更新 `dp` 数组，最终 `dp[m][n]` 即为结果，其中 `m` 和 `n` 分别是两个字符串的长度。

### 10月19日
- **学习内容**：最小栈、快速排序
- **完成题目**：
  1. 数组中的第K个最大元素：使用最小栈思路，维护一个大小为 k 的最小堆，遍历数组将元素加入堆，若堆大小超过 k 则弹出堆顶元素，最后堆顶元素即为第 K 个最大元素；也使用了快速排序思路，通过随机选择基准值，将数组分为两部分，根据基准值位置与 k 的关系缩小查找范围。
  2. 前k个高频元素：使用最小栈思路，先统计每个元素的频率，然后维护一个大小为 k 的最小堆，根据频率将元素加入堆，若堆大小超过 k 则弹出堆顶元素，最后堆中元素即为前 k 个高频元素。

### 10月20日
- **学习内容**：贪心算法
- **完成题目**：
  1. 买卖股票的最佳时机：使用贪心算法，遍历数组，维护一个当前遇到的最小股票价格，计算当前价格与最小价格的差值，不断更新最大利润。
  2. 跳跃游戏：使用贪心算法，维护一个能到达的最远位置。遍历数组，每次更新这个最远位置，如果在遍历过程中最远位置能到达或超过数组末尾，则说明可以到达最后一个位置。

### 9月30日
- **学习内容**：贪心算法
- **完成题目**：
  1. 跳跃游戏Ⅱ：使用贪心策略，维护当前能跳到的最远位置、上一次跳跃能覆盖的最远位置以及跳跃次数。遍历数组，不断更新当前能跳到的最远位置，当遍历到上一次跳跃能覆盖的最远位置时，说明需要进行下一次跳跃，此时更新跳跃次数并将上一次跳跃的最远位置更新为当前能跳到的最远位置。
  2. 划分字母组合：使用贪心算法，记录每个字母最后一次出现的位置。遍历字符串，维护当前区间的结束位置，对于每个字母，更新当前区间的结束位置为其最后一次出现位置的最大值。当遍历到当前区间的结束位置时，说明可以在此处进行划分，记录当前区间长度并开始新的区间。

### 9月30日
- **学习内容**：双指针
- **完成题目**：
  1. 接雨水：使用双指针的方法。维护左右两个指针以及左右两边的最大高度，遍历数组时，每次移动较矮的一侧指针，并计算当前位置能接的雨水量，累加到结果中。
